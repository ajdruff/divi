	...
	<?php
		...
		if ( isset( $et_one_font_languages[$site_domain] ) ) {
			printf( '%s { font-family: %s; }',
				...
				esc_html( $et_one_font_languages[$site_domain]['font_family'] )
			);
...
function et_aweber_submit_authorization_code() {
	if ( ! wp_verify_nonce( $_POST['et_admin_load_nonce'], 'et_admin_load_nonce' ) ) {
		die( __( 'Nonce failed.', 'et_builder' ) );
	}
	...
	try {
		...
		et_update_option( 'divi_aweber_consumer_key', sanitize_text_field( $consumer_key ) );
		et_update_option( 'divi_aweber_consumer_secret', sanitize_text_field( $consumer_secret ) );
		et_update_option( 'divi_aweber_access_key', sanitize_text_field( $access_key ) );
		et_update_option( 'divi_aweber_access_secret', sanitize_text_field( $access_secret ) );
...
if ( ! function_exists( 'et_divi_get_post_bg_inline_style' ) ) {
	function et_divi_get_post_bg_inline_style() {
		...
		if ( $post_use_bg_color ) {
			$inline_style = sprintf( ' style="background-color: %1$s;"', esc_attr( $post_bg_color ) );
		}
...
function et_pb_get_aweber_account() {
	...
	$consumer_key = sanitize_text_field( et_get_option( 'divi_aweber_consumer_key' ) );
	$consumer_secret = sanitize_text_field( et_get_option( 'divi_aweber_consumer_secret' ) );
	$access_key = sanitize_text_field( et_get_option( 'divi_aweber_access_key' ) );
	$access_secret = sanitize_text_field( et_get_option( 'divi_aweber_access_secret' ) );
